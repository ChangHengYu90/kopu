...
Page({
  ...
  onLoad: function (options) {
    ...
    that.setData({
      xmlxr: wx.getStorageSync('userxq').account,
      xmphone: wx.getStorageSync('userxq').phone
    })
  ...
  bindPickerChange: function (e) {
    ...
    this.setData({
      ...
      leixing: that.data.array[0][e.detail.value[0]] + '-' + that.data.array[1][e.detail.value[1]]
    })
  },
  //点击事件，点击弹出选择页
  dianji: function () {
  ...
  hqbiaoti: function (e) {
    var that = this
    that.setData({
      xmtitle: e.detail.value.replace(/\s+/g, '')
    })
  },
  // 获取地址
  hqdizhi: function (e) {
    var that = this
    that.setData({
      xmdizhi: e.detail.value.replace(/\s+/g, '')
    })
  },
  ...
  hqmianji: function(e) {
    var that = this
    that.setData({
      xmmianji: e.detail.value.replace(/\s+/g, '')
    })
  },
  // 获取地址
  hqprice: function (e) {
    var that = this
    that.setData({
      xmprice: e.detail.value.replace(/\s+/g, '')
    })
  },
  ...
  hqwuye: function(e) {
    var that = this
    that.setData({
      xmwuye: e.detail.value.replace(/\s+/g, '')
    })
  },
  // 获取联系人
  hqlxr: function (e) {
  ...
  submit:function(){
    ...
    let xmtitle = that.data.xmtitle
    let xmdizhi = that.data.xmdizhi
    let xmmianji = that.data.xmmianji
    let xmprice = that.data.xmprice
    let xmwuye = that.data.xmwuye
    let xmlxr = that.data.xmlxr
    let xmphone = that.data.xmphone
    let xmxqms = that.data.xmxqms
    let leixing = that.data.leixing
    let leixing2 = that.data.multiIndex
    ...
    if (qu == '' || qu.length < 1 || qu == undefined||qu==null) {
      qu = 0
    }
    ...
    if (xmtitle == '' || xmtitle == undefined || xmtitle == null) {
      wx.showToast({
        title: '请输入标题',
        icon: 'none',
        duration: 1000,
    ...
    if (xmdizhi == '' || xmdizhi == undefined || xmdizhi == null) {
      wx.showToast({
        title: '请输入地址',
        icon: 'none',
        duration: 1000,
    ...
    if (xmmianji == '' || xmmianji == undefined || xmmianji == null) {
      wx.showToast({
        title: '请输入面积',
        icon: 'none',
        duration: 1000,
    ...
    if (xmprice == '' || xmprice == undefined || xmprice == null) {
      wx.showToast({
        title: '请输入价格',
        icon: 'none',
        duration: 1000,
    ...
    if (xmwuye == '' || xmwuye == undefined || xmwuye == null) {
      wx.showToast({
        title: '请输入物业',
        icon: 'none',
        duration: 1000,
    ...
    let shuju = {
      userId: userid,
      houseName: xmname,
      housetype: parseInt(leixing2[0]) + 1,
      type: parseInt(leixing2[1]) + 1,
      MotherMoney: xmprice,
      address: dizhi,
      area: xmmianji,
      linkMobile: xmphone,
        ...
        wx.uploadFile({
          ...
          success(res) {
            ...
            if (i == uploadpic.length - 1) {
              ...
              shuju.img = img
              wx.request({
                url: that.data.href + 'api/House/XCXUserHouseAdd',
