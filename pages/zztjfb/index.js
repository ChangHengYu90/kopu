...
Page({
  ...
  hqdizhi: function (e) {
    var that = this
    that.setData({
      xqdizhi: e.detail.value.replace(/\s+/g, '')
    })
  },
  hqmianji: function (e) {
    var that = this
  ...
  hqxqjg: function (e) {
    var that = this
    that.setData({
      xqjg: e.detail.value.replace(/\s+/g, '')
    })
  },
  hqxqms: function (e) {
    var that = this
  ...
  submit:function(){
    ...
    let xqdizhi = that.data.xqdizhi
    let mianji = that.data.mianji
    let xqjg = that.data.xqjg
    let xqms = that.data.xqms
    let titl = that.data.title
    ...
    if (titl == '' || titl == undefined || titl==null){
        wx.showToast({
          title: '请填写标题',
          icon:'none',
          duration:1000,
    ...
    if (xqdizhi == '' || xqdizhi == undefined || xqdizhi == null) {
      // wx.showToast({
      //   title: '请填写需求地址',
      //   icon: 'none',
      //   duration: 1000,
    ...
    if (xqjg == '' || xqjg == undefined || xqjg == null) {
      // wx.showToast({
      //   title: '请填写需求价格',
      //   icon: 'none',
      //   duration: 1000,
    ...
    wx.request({
      url: that.data.href + 'api/User/ZZTwitterAdd',
      method: 'POST',
      data: data,    //参数为键值对字符串
      header: {
        //设置参数内容类型为x-www-form-urlencoded
