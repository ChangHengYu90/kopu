...
Page({
  ...
  data: {
    ...
    href: app.data.href,
    array: [['厂房', '商铺', '土地', '办公'], ['出租', '出售', '求租', '求售']],
    multiIndex: [0, 0],
    array2: ['100', '200', '300', '400'],
    array3: ['20元以下', '20-50元', '50-80元', '80-100元', '100-120元', '120-150元', '150元以上'],
    index:0,
    index2: 0,
    index3: 0,
    sheng: [],//获取到的所有的省
  ...
  getlist(){
    var that = this
    let seotitle = that.data.seotitle
    ...
    if (seotitle == '' || seotitle == undefined || seotitle == null){

    }else{
      data.seotitle = seotitle
    }
    if(dizhi!='区域'){
      data.townid = that.data.povinceId
      data.districtID = that.data.cityId
      if (that.data.areaId == '' || that.data.areaId == undefined || that.data.areaId == null ){
            
      }else{
        data.jd = that.data.areaId
      }
    }
    if (mianji!='面积'){
      data.area = mianji
    ...
    if (jiage != '价格') {
      if (that.data.leixing =='出租'){
        data.MotherMoney = jiage
      }else{
        data.MotherMoney = jiage
      }
      data.price = mianji
    ...
    console.log(data)
    wx.request({
      url: that.data.href + 'api/House/GetMyClass',
      method: 'POST',
      data: data,    //参数为键值对字符串
  ...
  golookxq: function (e) {
    ...
    let content = that.data.sjlist
    let a = content[index].imgUrl
    console.log(a)
    content[index].imgUrl = [a]
  ...
  bindPickerChange: function (e) {
    ...
    this.setData({
      ...
      leixing: that.data.array[1][e.detail.value[1]],
      housetype: parseInt(e.detail.value[0])+1,
      typeid: parseInt(e.detail.value[1]) + 1,
      jiage:'价格',
      page:1
    })
    that.getlist()
  ...
  bindPickerChange2: function (e) {
    ...
    console.log('picker发送选择改变，携带值为', e.detail.value)
    this.setData({
      mianjiid: e.detail.value,
      mianji: that.data.array2[e.detail.value],
      page: 1
    })
    that.getlist()
  },
  bindPickerChange3: function (e) {
    ...
    console.log('picker发送选择改变，携带值为', e.detail.value)
    this.setData({
      jiageid: e.detail.value,
      jiage: that.data.array3[e.detail.value],
      page: 1
    })
    that.getlist()
  },
