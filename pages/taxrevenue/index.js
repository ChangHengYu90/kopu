...
Page({
  ...
  onLoad: function (options) {
    ...
    if (shuju){
      that.setData({
        xmname: shuju.ptitle,
        xmxqms: shuju.pcontent,
        xmphone: shuju.plinkphone,
        xmlxr: shuju.plinkpeople,
        isxiugai:1,
        xmid: shuju.id,
        otherO: shuju.otherO,
      })
  ...
  submit: function () {
    ...
    if (uploadpic.length > 0) {
      ...
      //         shuju.img = img
      //         wx.request({
      //           url: that.data.href + 'api/User/XCXZSTwitterAdd',
      //           method: 'POST',
      ...
      if (that.data.isxiugai==0){
        wx.request({
          url: that.data.href + 'api/User/XCXZSTRevenueAdd',
          method: 'POST',
          data: shuju,    //参数为键值对字符串
          header: {
            //设置参数内容类型为x-www-form-urlencoded
        ...
        wx.request({
          url: that.data.href + 'api/User/XCXZSTRevenueChange',
          method: 'POST',
          data: shuju,    //参数为键值对字符串
          header: {
            //设置参数内容类型为x-www-form-urlencoded
